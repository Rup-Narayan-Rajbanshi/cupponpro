...
class SalesItemDetailView(APIView):
    ...
    def get(self, request, salesItem_id):
        salesItem_obj = SalesItem.objects.filter(id=bill_id)
        if salesItem_obj:
            serializer = Serializer(salesItem_obj[0], context={'request':request})
            data = {
                'success': 1,
                'salesItem': serializer.data
            }
            ...
            data = {
                'success': 1,
                'bill': "Sales Item doesn't exist."
            }
            return Response(data, status=404)
    
    def delete(self, request, salesItem_id):
        salesItem_obj = SalesItem.objects.filter(id=salesItem_id)
        if salesItem_obj:
            try:
                salesItem_obj[0].delete()
                data = {
                    'success': 1,
                    'salesItem': 'Sales Item deleted successfully.'
                }
                return Response(data, status=200)
            except:
                data = {
                    'success': 0,
                    'salesItem': 'Sales Item cannot be found.'
                }
                return Response(data, status=400)
        else:
            data = {
